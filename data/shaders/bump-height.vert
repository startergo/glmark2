...
void main(void)
{
    ...
    // bitangent.
    NormalEye = normalize(vec3(NormalMatrix * vec4(normal, 0.0)));
    TangentEye = normalize(vec3(NormalMatrix * vec4(tangent, 0.0)));
    BitangentEye = normalize(vec3(NormalMatrix * vec4(cross(normal, tangent), 0.0)));
