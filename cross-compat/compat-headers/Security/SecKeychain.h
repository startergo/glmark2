...
/*
 * glmark2 10.6 cross-compat shim (shadow copy of the 10.6 SDK header).
 *
 * The stock header defines AUTH_TYPE_FIX_ with signed shifts; on little
 * endian 'ntlm'-style multi-char literals overflow int when shifted left,
 * which is UB and therefore not an integral constant expression in C++.
 * Modern clang rejects the enum initializers in ObjC++ compilations.
 * The unsigned-arithmetic variant below computes identical values and is
 * a valid constant expression.
 */
#ifdef __LITTLE_ENDIAN__
#define AUTH_TYPE_FIX_(x) \
        ((int)((((unsigned)(x) >> 24) | (((unsigned)(x) >> 8) & 0xff00u) | \
               (((unsigned)(x) << 8) & 0xff0000u) | (((unsigned)(x) & 0xffu) << 24))))
#else
#define AUTH_TYPE_FIX_(x) (x)
#endif
/* Skip the stock (broken-in-C++) macro definition below. */
#define _SECURITY_SECKEYCHAIN_COMPAT_MACRO_ 1
...
*/
/* Original definition removed; see the compat shim at the top of this file.
#ifdef __LITTLE_ENDIAN__
#define AUTH_TYPE_FIX_(x) \
